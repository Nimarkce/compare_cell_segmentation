...
def RmIpynbCheckp(Bad_dir):
    try:
        shutil.rmtree(os.path.join(Bad_dir, ".ipynb_checkpoints"))
    except:
        print("no such file: {}".format(os.path.join(Bad_dir, ".ipynb_checkpoints")))
...
def get_train_df(train_dir, save_name):
    RmIpynbCheckp(train_dir)
    train_ids = list(next(os.walk(train_dir)))[1]
    train_df = []
    for id_ in train_ids:
        print("1-we are now at:{}".format(id_))
        ...
        try:
            ...
            print("the false occurs in:{}".format(id_))
#            os.system("rm -rf {}".format(id_))
#       gray = is_gray(image)
#        shape = image.shape[:2] if gray else image.shape
#        masks = []
#        for mask_fl in glob.glob(os.path.join(train_dir, id_, 'masks', '*.*')):
...
def get_test_df(test_dir, save_name='test_df'):
    RmIpynbCheckp(test_dir)
    test_ids = sorted(list(os.listdir(test_dir)))
    test_df = []
    for id_ in test_ids:
        print("2-we are now at:{}".format(id_))
        try:
            id_ = id_[:-4]
