...
def get_df():
    ###################################################################
    data_dir = '..//TNBC_NucleiSegmentation'
    ...
    for image_folder in image_folders:
        ...
        for image_fl in image_fls:
            ...
            df.append({'image':image, 'mask':mask,
                       ...
                       'shape':image.shape})    #df there is a dictionary.

    df = pd.DataFrame(df)   #convert df to DataFrame
    save_to_cache(df, 'TNBC')

if __name__ == "__main__":      # if run the current file, the __name__="__main__"
    get_df()